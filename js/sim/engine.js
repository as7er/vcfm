...
export class SimEngine {
  ...
  _gkDistribute(a) {
    ...
    // —— 大脚出边线（界外球）：恢复**一小部分**「被迫 / 失准」的解围 ——
    //
    // 历史（本函数头注释 + 2530 行注释）：旧实现瞄 x=22/78、力量 30~44，落地仍带 ~20 速度
    // ⇒ **经常**直接滚出边线（用户当时能看见「门将开大脚出界」）。
    // 2026-09 的修法把落点夹进 `x∈[30,70]` 并压低残速，于是走向另一个极端：
    // **整场 0 次界外球**（实测 17 场 × 90 分钟 = 0 次；
    // 见 `docs/measurements/restart-placement-root-cause-2026-09-22.txt` §5②）。
    //
    // 真实里门将大脚既不「经常出界」也不「永不越线」—— 这里取中间档：
    // 一小部分解围**斜向**飞出边线（落点纵深照旧，只是横向越过边线），判给对手界外球。
    // 受压时比例更高（越贴身越容易踢呲）。落点瞄准**边线外 12 格**，让过线点确定。
    //
    // ⚠ 只改「往哪瞄」，不改力量/弹道：出界那一脚看起来仍是一记普通大脚。
    // 概率与落点都取「真实里门将大脚出界」的量级：真实门将每场约 20 次大脚，
    // 其中约 5~10% 出边线 ⇒ 本档给 0.11（受压）/ 0.055（正常），落点瞄边线外 6 格。
  ...
  _penaltyKick(team) {
    ...
    // 基线 0.77 → 0.85（2026-09-22）：**只动这一个常数**，斜率与结构一字未改。
    //
    // 为什么：旧基线让真实俱乐部（`createWorld` 全量世界，400 组随机对阵、真入口
    // `_penaltyKick`）的平均命中率只有 **67.09%**（P10 63.5% / P90 70.9%，
    // 分布 60-65%: 95 组 · 65-70%: 231 组 · **75-80%: 仅 1 组**），
    // 而现实足球约 **75~80%**。更直接的矛盾是：**本仓库自己的点球 xG 硬编码就是 0.76**
    // （`js/match.js:1200`、`js/match-analysis.js:40`）—— 引擎实际转化率与它自己
    // 声明的 xG 差了 9 个百分点，战报数字与画面互相打架。
    //
    // 为什么改基线而不是改斜率/门将权重：斜率 `0.34` 与门将项 `0.22` 决定了
    // **能力区分度**（主罚者 1→20 极差 18.9pp、门将 1→20 极差 11.1pp），
    // 那是设计的一部分；命中率总量偏低是**基线**问题。动基线 ⇒ 能力区分度一点不减，
    // 点球**判罚频次**也完全不动（判罚侧一行没碰）。
    //
    // 实测（`scripts/penalty-conversion-verify.mjs`，确定性）：平均 67.09% → ≈75.3%。
    // 上夹 0.9 在新基线下仍几乎不触发（顶级射手 vs 门将 1 也只有 ~0.888），
    // 下夹 0.55 会真咬住弱主罚者 + 强门将（这本来就是想要的行为）。
    const pScore = clamp(0.85 + (penSkill - 0.6) * 0.34 - save * 0.22, 0.55, 0.9);
    const r = this.random();
    const outcome =
      r < pScore
        ? "goal"
