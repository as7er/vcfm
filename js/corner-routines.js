...
export function buildCornerRoutine({ agents, team, takerId, ball, now, attackDirection, goalY }) {
  ...
  forward.slice(0, 3).forEach((a, i) => {
    ...
    positions.set(a.id, {
      ...
      y: target.y - attackDirection * (i === 0 ? 4 : 5) / MY,
      kind: "runner",
    });
    runs.set(a.id, target);
  });
  const support = [at(side * 9, 9.5), at(-side * 8, 12), at(-side * 1, 18), at(side * 15, 21)];
...
export function cornerDelivery(routine, agents, taker) {
