...
2. **【主线·已选定 (b)，本轮实施 → 结果：设计失败】** boxSec 取舍（交接 §5 第 1 条）：
...
- **重标到 960 是错的**：真实效应约 **+167s**（847.5 − 680.0），不是 6 场显示的 +199s。
  按 6 场数字定天花板 = 把护栏放宽到**超出数据支持**的程度。
- **守 850 同样是拍脑袋**：它是抛硬币。
- **`margin2` 的越位 1.88 ✅、纵深改善 ✅ 是确定的**（那两项的样本足够）；
  唯独 boxSec 这一条**判不了**。

⇒ **结论：boxSec 这一条要变成可判，必须先给 boxSec 本身做噪声标定**
（与直塞那条同型的方法：跑 N 批互不重叠的窗口，测批间标准差）。
**在此之前不落引擎。** 这是「引擎不动直到完全验证」这条纪律的直接应用。
