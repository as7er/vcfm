...
#### 2.1.1 实测确证：摆脱率**不含属性信息**（2026-09-18 夜新增）

上述是静态阅读的推论。为把它变成**测量**，新建探针
`scripts/_one-on-one-dribble-probe.mjs`（纯只读：读 `engine.events` + 每 0.1s 采样
持球者与最近对手的几何关系，不插桩、不消费随机数）。

**关键方法学约束（决定了必须造差异档）**：现有全部探针的球队都是
「所有属性同值」（能力 15 ⇒ `dribbling`/`vision`/`passing` 全部 = `norm(15)` = 0.76）。
同值档下**球星与普通球员在构造上就相同**，任何"按属性结算"的机制都测不出来。
所以本探针显式构造**差异档**：核心球员 `dribbling` 18 / `vision` 18，
普通球员 `dribbling` 9 / `vision` 9 —— **整整 2 倍差**。
...
**uniform 档是决定性的反证**：那一档**所有属性都相同**（含核心与普通），
摆脱率却仍有 0.3% vs 0.95% 的差异 ⇒ 这个差异**根本不来自属性**，
只来自「谁更常被贴身、谁面对的防守密度不同」。

**那 1.2% 的"摆脱"是什么？** 是**几何副产品**：防守者自己的移动惯性没跟上、
抢断冷却（`tackleCdUntil`，36~60s 量级）、或球权转移的瞬间 ——
**不是任何结算的"过人成功"**。

**一个被实测推翻的初版推测（诚实记录）**：我原先猜「贴身时以回避为主」（因为
`dribble` 权重乘 `(1 - pressure*0.55)`）。**实测否定**：
贴身时向前推进占 **60.3%**、回避只占 **17.1%**。原因是 `pressure` 的定义
（`_pressureOn`：`clamp(1 - nearest/12, 0, 1)`）决定"贴身"（≤3m）本身就包含
**2-3m 这个常态盯防距离**（占 46.8% 的样本），此时下方仍有充足空当可推进。
⇒ 但**向前推进 ≠ 主动过人** —— 前者是"前方有空当就走"，后者是"顶着防守者把球带过去"。
判据在摆脱率的**属性差**，不在这个前进/回避比。

**原始读数归档**：`docs/measurements/probe-one-on-one-{tiered,uniform}-2026-09-18.txt`。

⇒ **结论：§2.1 的静态推论经实测确证。** 「梅西式一对一突破」需要一条
**新原语**（持球者主动发起 + 按 `dribbling`/`balance` vs `tackling`/`marking` 结算 +
产生 `beat` 事件），而不是给现有 `dribble` 调权重 ——
...
6. **【机制侧·新】中场不交换场地**（2026-09-18 深夜核查，**用户要求记入待办**）：
   **当前引擎全程不换边** —— 主队整场攻同一侧球门。

   **三处证据（都已核实）**：
   | 位置 | 现状 |
   |---|---|
   | `engine.js:735` `attackDir(team)` | `return team === "home" ? -1 : 1;` —— **签名里没有半场参数** |
   | `adapt.js:83` `resyncSimAfterHalfTime()` | 函数名带 HalfTime，但只做球员/阵型同步；注释写「**尽量保位置连续性**」⇒ 本就没打算换边 |
   | `matchview.js:2068` `showSecondHalfKickoff()` | 只把球重置到中圈（`ball.tx = ty = 50`）+ 换横幅文案 |

   **为什么它是真实感缺口**：真实足球中场交换半场（教练要重新考虑阳光/风向、
   边线球与角球的主客方向互换）。项目最高原则是「应尽可能接近现实足球」。

   **为什么优先级不高**：**它不影响任何已标定的指标**。球场左右对称，
   `attackDir` 只是个方向符号 ⇒ 进球/越位/传球分布/boxSec 全都不受"攻哪一边"影响。
   纯观感问题，不阻塞任何主线。

   **若要补，`attackDir` 是唯一真源**（全引擎 40 处调用都走它），但⚠ **必须同时处理
   三处连带**，否则会出 bug：
   1. `targetGoalY()`（`:739`）与 `attackDir` 同源，须同步翻转；
   2. **`offsideDir`（`:3223`）与球门坐标（`:4915`、`:4990`）是硬编码
      `HOME_GOAL_Y`/`AWAY_GOAL_Y` 的两分支写法**，**不会**靠 `attackDir` 自动覆盖；
   3. 换边瞬间的球员坐标需镜像（`x→100-x, y→100-y`）——
      `resyncSimAfterHalfTime` 的「保位置连续性」注释说明现在刻意没做。

   ⚠ **一个天然的自检**：因为球场对称，改完之后**所有已标定的审计读数都不应该变**。
   这本身就是一条现成的零回归断言（与突破原语 §7.2 第 4 步同型）。

   **建议**：等突破原语那条线走完再说（不阻塞）。若要做，宜与
   `resyncSimAfterHalfTime` 一并处理，避免"换边但位置不镜像"的状态不一致。
