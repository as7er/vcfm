...
**下一轮必须换机制，而不是换数值。** 两个候选方向（均未验证，仅作记录）：

1. **从「防守端」压跨度。** 进攻三区跨度 47.3 m 的上端是 ATT 98.0 m，
   而它是被**对手的最后一名后卫**钳住的（`_clampOffside`）。对手在本方三区防守时
   后卫线只有 31.9 m 深（HEAD 实测，未控球帧）——**对手站得越深，我们的前锋就被钉得越深**。
   让防守方在球进入本方三区时把防线**顶出去**（真实球队的常规做法），
   可以在**完全不动进攻方中卫线**的前提下压短跨度。
