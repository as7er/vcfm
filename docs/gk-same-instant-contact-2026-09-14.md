...
### 7.5 顺带量到的：射门下降的归因（⚠ 本节曾写错，已更正）

同种子、同审计（标准档 24 场）。用 `scripts/_pin-engine-revision.mjs` 逐段钉住历史 revision：

| 引擎 | 总脚 | 总球 | 转化% | <16 m 脚/转化 | 16–22 m 脚/转化 |
|---|---:|---:|---:|---|---|
| `5f1d152`（v253） | 735 | 70 | 9.5 | 492 / 10.2% | 218 / 8.7% |
| `10378e3`（**射门频率标定**） | 671 | 68 | 10.1 | 479 / 10.2% | 172 / 10.5% |
| `75d9e4c`（中卫线前压**之前**） | **671** | **68** | 10.1 | 479 / 10.2% | 172 / 10.5% |
| HEAD（+ 中卫线前压三笔） | 647 | 66 | 10.2 | 460 / **11.5%** | 164 / 7.3% |

- **⚠ 更正**：本文早先版本把 735 → 647 的射门下降整体归因给中卫线前压三笔，**这是错的**。
  `10378e3` 与 `75d9e4c` **逐位相同**，说明 **735 → 671 的 −64 脚（−8.7%）是有意的射门
  频率标定**（见 [match-shot-frequency-2026-09-13.md](match-shot-frequency-2026-09-13.md)，
  目标是接近真实英超约 26 次/场）。
- 中卫线前压三笔（`e6567a8`/`0692c6a`/`2bd9fa5`）实际只贡献 **671 → 647，即 −24 脚（−3.6%）
  与 −2 球**，且把转化结构从 16–22 m（10.5% → 7.3%）搬到了 <16 m（10.2% → 11.5%）。
  这个量级在噪声内，不是问题。
- **教训**：把两笔改动之间的差值直接归因给后一笔，会系统性放大后一笔的罪名。
  用 `scripts/_pin-engine-revision.mjs` 逐段钉住 revision 才能分离归因。
