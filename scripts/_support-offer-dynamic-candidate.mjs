// Keep tactical depth, refine the available lateral windows, and retire an
// offer when its passing lane closes, even while the same holder has the ball.
...
function applySupportOffer(a, owner, previous) {
  ...
  if (offer && b.owner) {
    // A new holder needs a fully open line. A continuing offer keeps a smaller
    // exit threshold to avoid reversing for harmless changes inside its margin.
    ...
    if (!receivingSpace || this._laneSafety(owner, a, offer.x, offer.y) < minimumLane / 8 ||
...
process.on("exit", () => console.log(JSON.stringify({ supportOfferDynamicCandidate: { loadedEngineSha256 } })));
