// A supporting player's depth comes from the tactical/third-player-run plan.
// Find a lateral passing window without turning the offer into a retreat run.
  ...
  if (b.owner === owner.id && b.state === "held" && settled && gap(b, target) >= 6 && gap(b, target) < 20 &&
    ...
    for (const offset of [2, 4, 6]) {
      for (const side of [preferredSide, -preferredSide]) {
        const x = target.x + offset * side / mx;
        const y = anchor.y;
        const spot = { x, y };
        if (x < 3 || x > 97 || y < 3 || y > 97 || gap(spot, anchor) > 6 + 1e-7 ||
            gap(b, spot) >= 20 || this._isOffsidePosition(a.team, spot)) continue;
...
process.on("exit", () => console.log(JSON.stringify({ supportOfferLateralCandidate: { loadedEngineSha256 } })));
