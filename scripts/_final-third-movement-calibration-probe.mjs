...
//     ① `PROBE_BAND.goals` —— **本口径自己的护栏**（2026-09-20 起带噪声依据）：
//        落在带内 = 与前三个版本同量级；越出 = 需要解释（真回归 或 真增益）。
//     ② `GATE.goals` —— 审计口径的真实带，**本探针不能自证**，
//        必须另跑 `node scripts/match-realism-audit.mjs 24` 才能判。
//
// 🔴 2026-09-20 升级（原为三点手画的软标注，已作废）：
//   旧值 `[2.2, 2.7]` 是拿 v251/v252/今日 三点（2.50/2.35/2.38）**手画**的，
//   半边只有 0.25 —— **没有人验过它相对噪声有多宽**。按本仓护栏纪律
//   （AGENTS.md「护栏纪律」：带宽必须由实测批间 SD 决定，不许拍脑袋），
//   2026-09-20 用新探针 `scripts/_goals-noise-calibration-probe.mjs`
//   **逐字沿用 beat / boxSec 的噪声标定方法型**（8 批 × 12 场 = 96 场，种子 372000..372095）
//   实测本口径噪声：
//
//     | 量 | 读数 |
//     |---|---|
//     | 进球/场 基线（双方合计） | **2.854** |
//     | 逐场 SD (`SD_match`) | **1.583**（单场 0~8，明显长尾，零球场占 4.2%） |
//     | 批间 SD (`SD_batch`, 12 场/批) | **0.471**（批内均值 2.000~3.583，极差 1.583） |
//
//   带宽推导（不手调）：`2 × SD_batch = 0.942` ⇒ 半边 **0.95**（向上取到 0.05 的整数倍）
//   ⇒ `band = [2.854 − 0.95, 2.854 + 0.95] = [1.90, 3.80]`。
//
//   ⚠ **对照旧软标注 `[2.2, 2.7]`（半边 0.25）：实测噪声是它的 3.8 倍。**
//     也就是说旧带**比噪声还窄** —— 它过去没报警，只是那几次读数恰好都落在 2.35~2.50，
//     纯属运气；只要有任意一次 12 场读数漂到 2.8 或 2.1，它就会误报。
//     这正是 boxSec `850` / 直塞 `[0.5, 12]` 那个坑的同型复发，**已修**。
//
//   ⚠ **判决力边界（必须连同护栏一起读）**：`SD_match = 1.583` ⇒ 要辨出
//      1 球/场 的效应每组需 ~21 场；**0.5 球/场 需 ~81 场**。本探针默认只跑 6 场，
//      ⇒ **它连 2 球/场 的效应都判不了**（需 ~6 场/组，勉强）。所以本带的正确定位是
//      **"防大幅回归"的红线，不是"能判 0.5 球"的仪器**。要判小效应请另跑
//      `node scripts/_goals-noise-calibration-probe.mjs` 补样，不要指望这条带。
const PROBE_BAND = {
  // 2026-09-20：96 场噪声标定（8 批 × 12 场，种子 372000..372095）
  //   SD_batch(12 场/批) = 0.471 ⇒ 2SE = 0.942 ⇒ 半边取 0.95
  //   band = [2.854 − 0.95, 2.854 + 0.95] = [1.90, 3.80]
  //   归档：`docs/measurements/probe-goals-noise-calibration-2026-09-20.txt`
  goals: [1.9, 3.8],
  /** 本带的历史出处（打印用；不要拿它当护栏） */
  goalsLegacySoft: [2.2, 2.7],
  goalsBaseline: 2.854,
  goalsHalfWidth: 0.95,
  goalsSdBatch: 0.471,
};
...
for (const level of LEVELS) {
  ...
  const warn = [];
  // ① 本口径护栏（2026-09-20 起由 96 场噪声标定定带宽，不再是软标注）
  //    ⚠ 判决力边界：本探针默认 6 场，只能抓「大幅回归」；判 0.5 球/场 需 ~81 场。
  if (r.进球 < PROBE_BAND.goals[0] || r.进球 > PROBE_BAND.goals[1]) warn.push("进球越本口径护栏");
...
console.log({
  ...
  "进球 本探针口径护栏（96 场噪声标定）": `${PROBE_BAND.goals.join("~")} ← 基线 ${PROBE_BAND.goalsBaseline} ± 0.95（2×SD_batch ${PROBE_BAND.goalsSdBatch}）`,
  "进球 旧软标注（已作废，仅留档）": `${PROBE_BAND.goalsLegacySoft.join("~")} ← 三点手画，半边 0.25 比噪声窄 3.8×`,
...
console.log(
  [
    ...
    "· 想看到的方向：boxSeconds / 禁区触球下降、直塞上升、[2c] 纵深三列上移，而进球留在本口径护栏内。",
    `· ⚠ 进球判据有两条口径，别混用：本探针口径护栏 ${PROBE_BAND.goals.join("~")}`,
    `  （2026-09-20 由 96 场噪声标定定带宽：基线 ${PROBE_BAND.goalsBaseline}、半边 0.95 = 2×SD_batch ${PROBE_BAND.goalsSdBatch}；`,
    `  旧软标注 ${PROBE_BAND.goalsLegacySoft.join("~")} 半边仅 0.25，比噪声窄 3.8×，已作废）；`,
    `  审计口径真实带 ${GATE.goals.join("~")}（能力 13、含强弱，须另跑 match-realism-audit）。`,
    "  ⚠ 反面教训：拿审计口径的 2.5 当本探针的门 = 预设一个本口径从未满足的门槛，任何档位都会被判失败。",
    `  ⚠ 判决力边界：本探针默认 6 场，SD_match ${"1.583"} ⇒ 只能抓「大幅回归」；判 0.5 球/场 需 ~81 场。`,
