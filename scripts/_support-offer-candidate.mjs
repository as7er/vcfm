// Process-local continuation of the support-lane experiment. A selected outlet
// survives ordinary target refreshes, and ends with its possession/task context.
...
function applySupportOffer(a, owner, previous) {
  ...
  if (b.owner === owner.id && b.state === "held" && settled && gap(b, target) >= 6 && gap(b, target) < 20 &&
    ...
    if (choice) offer = { ...choice, at: this.t, attackSince: this._teamAttackSince[a.team],
      origin: { x: b.x, y: b.y } };
  }
  if (!offer) return;
  a.tx = offer.x;
...
process.on("exit", () => console.log(JSON.stringify({ supportOfferCandidate: { loadedEngineSha256 } })));
