...
// ⚠ 口径警告（2026-09-14 补）：下面这一列名字叫「DEF→ATT 跨度」，但实现是 `att - cb`，
//    **只算中卫线到锋线，不含边卫**。所以本探针对「远侧边卫拖在后面」这类缺陷**完全失明**：
//    2026-09-14 实测控球时球侧边卫 93 m、远侧边卫 38 m，本探针的跨度列仍是 45.3 m（无变化），
//    而把边卫算进去的真实队形跨度是 58.6 m。要看整条后防线请用
//    `scripts/_attack-block-length-breakdown-probe.mjs`（复刻审计口径、含边卫、并拆中卫/边卫）。
//    另一处偏差：`median()` 对 2 个元素取的是 index 1 = **较大值**，所以 FB 行是「更靠前那名边卫」，
//    不是两名边卫的中位。两处都不是 bug（本探针只诊断中卫线），但读数时别当成全队口径。
