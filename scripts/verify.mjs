...
const checks = [
  ...
  // 「隐藏/唤出」常驻开关（2026-09-22 v288）：用户报「球场上方显示不全，能不能
  // 给底部功能区加隐藏/唤出」。查清两件事：
  //   ① 隐藏/唤出 v284 **已经存在**（点空白球场），只是屏幕上没有入口 ⇒ 发现不到；
  //   ② 「上方显示不全」的真因是**顶部比分浮层**压住球场盒顶 37px（19~26px 是真草坪
  //      + 整条上看台带），与底部功能区无关（只隐藏底部，顶上照样盖 37px）。
  // 本护栏守的是**接线**（按钮标记 / 事件委托 / stopPropagation / 状态同步 / CSS 层级）。
  // ⚠ 行为验证在 `scripts/_chrome-toggle-browser-check.mjs`（真实 Chromium，
  //   3 视口 51 项；含「点一次不能切两下」这条只有真点一次才看得出的判据）。
