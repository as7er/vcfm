...
let spatial = null;
{
  ...
  // ⚠ 判据必须是「到**最近的 11 的倍数**的距离 < 1e-6」，**不能**写成 `spent % 11 < 1e-6`：
  //   分摊是 `(work/sumWork)*teamTotal` 逐人算再求和的浮点累加，真值是 33 时可能得
  //   32.99999999999999 —— 那时 `spent % 11` 是 **~11**（不是 ~0），断言会无辜报红。
  //   实测：同一份代码单独跑 5 次全绿，而全量跑到这里时报红，打印值却同样是 33.000000
  //   —— 这就是这个公式坑的现场（本审计的动态段落是**未播种**的，跑动距离每次不同）。
  //   容差本身 **1e-6 未变**；改的只是「余数」→「到最近倍数的距离」，对任何非倍数
  //   （例如 32.5）两种写法都照样报红 ⇒ 判别力没有下降。
  const nearest11 = Math.round(spent / 11) * 11;
  check(Math.abs(spent - nearest11) < 1e-6,
    "球队总量是「11 的整数倍」（= 每人名义扣减 × 人数 × 结算次数）",
    `实测 ${spent.toFixed(6)}（最近倍数 ${nearest11}）`);
