/**
 * Generate js/avatar-assets.js with embedded manifest + trait matching.
 * Run: node scripts/build-avatar-assets-module.mjs
 */
import fs from "fs";
...
const manifest = JSON.parse(
  fs.readFileSync(path.join(root, "assets/player-avatars/manifest.json"), "utf8")
);
const builtin = JSON.stringify(manifest, null, 2);
...
const BUILTIN_MANIFEST = ${builtin};
...
const outPath = path.join(root, "js/avatar-assets.js");
fs.writeFileSync(outPath, code, "utf8");
console.log("wrote", outPath, "avatars=", manifest.avatars.length, "bytes=", code.length);
